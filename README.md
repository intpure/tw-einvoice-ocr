...
**用真實 Aqua 收據驗證過**（`05_客戶案子\001_Aqua\01_來源\收據範本\`）：高鐵票
樣本（memory `aqua-ocr-vision-fallback-fix-2026-07-22.md` 記錄的「車次 1550 vs
票價 1250」誤判案例原型）經 row-scoped 擷取正確拿到 `tax_inclusive=1250`，
沒有被同張票上文字距離較近的車次號碼 1550 帶偏——這是這次修復要解決的真實
案例，已用真實文件跑通，不只是合成測試資料。同一批樣本也發現兩個未解限制：
①另一張同款高鐵票因 OCR bounding box 把「NT$」跟金額數字拆進不同視覺行，
沒有共同的標籤關鍵字可比對，抓錯；②熱感應收據（淡色印刷+桌面木紋背景）的
`scan_document()` 前處理仍不穩定，已加 `_crop_to_document()`（亮度二值化抓
收據輪廓裁掉背景木紋噪點）緩解但未完全解決，遇到低對比照片建議人工確認
（設計上本來就是 status="待審"，非全自動）。

**用 Aqua 真實190頁批次（`Aqua_2026-08_發票整理` 彙整檔）再驗一輪**：抽測
`昕妍生技_2026-08_原始單據.pdf` 第1頁（一張澄星整合行銷「電子發票證明聯」，
QR 解碼確認回 None，走 OCR 備援）——row-scoped 三個金額全部正確：
`tax_exclusive=4762`／`tax_amount=238`／`tax_inclusive=5000`，逐字對照原圖三個
標籤（銷售額合計/營業稅/總計）皆吻合，僅賣方名稱因字元辨識誤差被讀成「涂星」
（原文「澄星」，單字元誤判，非欄位擷取邏輯問題）。同批也抽到兩份完全不在
本引擎設計範圍內的真實文件：政府制式「銷貨退回/進貨退出或折讓證明單」（手寫
表格，非「標籤:數值」單行版面）與銀行商務卡消費對帳單——兩者 OCR 皆正確
回傳空值/0，沒有硬湊假數字，符合設計預期（rows 找不到符合標籤的行就是找不到，
不是漏洞）。
